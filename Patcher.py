...
class Patcher:
    ...
    def __init__(self, imsize, patchsize, step, nopartials, contatedges):
        self.patchsize = float(patchsize)
        self.step = step
        self.imsize = imsize
    ...
    def im2patches(self, img):
        ...
        for ix in range(0, self.imsize[0], self.step):
            for iy in range(0, self.imsize[1], self.step):
                ...
                ptc = img[ix:ix + int(self.patchsize), iy:iy + int(self.patchsize)]

                if ((ptc.shape[0] != int(self.patchsize)) or (ptc.shape[1] != int(self.patchsize))) and self.nopartials:
                    pass
                else:
                    ptchs.append(ptc)
    ...
    def patches2im(self, patches, combsq=False):
        ...
        for ix in range(0, self.imsize[0], self.step):
            for iy in range(0, self.imsize[1], self.step):
                ...
                tt = tmp[ix:ix + int(self.patchsize), iy:iy + int(self.patchsize)]

                if ((tt.shape[0] != int(self.patchsize)) or (tt.shape[1] != int(self.patchsize))) and self.nopartials:
                    pass
                else:
                    tmp[ix:ix + int(self.patchsize), iy:iy + int(self.patchsize)] = tmp[ix:ix + int(self.patchsize),
                                                                          iy:iy + int(self.patchsize)] + patches[ctr]
                    ctr = ctr + 1
        ...
        for ix in range(0, self.imsize[0], self.step):
            for iy in range(0, self.imsize[1], self.step):
                ...
                if ((tt.shape[0] != int(self.patchsize)) or (tt.shape[1] != int(self.patchsize))) and self.nopartials:
                    ...
                    tmp[ix:ix + int(self.patchsize), iy:iy + int(self.patchsize)] = tmp[ix:ix + int(self.patchsize),
                                                                          iy:iy + int(self.patchsize)] + 1

        if (tmp == 0).any():
            print(
                "KCT-WARNING: the selected patching scheme does not allow covering of all the image! Some pixels are not in any of the patches.")
