...
def vaerecon_direct(us_ksp, sensmaps, uspat, lat_dim=60, patchsize=28, contRec='', parfact=10, num_iter=302, regiter=15,
             reglmb=0, regtype='TV', usemeth=1, stepsize=1e-4, mode=[], z_multip=1.0, vae_model='', logdir=''):
    # set parameters
    # ==============================================================================
    ...
    x_rec, x_inp, funop, grd0, sess, grd_p_x_z0, grd_p_z0, grd_q_z_x0, grd20, y_out, y_out_prec, z_std_multip, op_q_z_x, mu, std, grd_q_zpl_x_az0, op_q_zpl_x, z_pl, z = definevae(
        lat_dim=lat_dim, patchsize=patchsize, mode=mode, vae_model=vae_model, batchsize=parfact * nsampl)
    ...
    def likelihood_grad(us):
        ...
        grd0eval = grd0.eval(
            feed_dict={x_rec: np.tile(usabs, (nsampl, 1)), z_std_multip: z_multip})  # ,x_inp: np.tile(usabs,(nsampl,1))
    ...
    def likelihood_grad_patches(ptchs):
        ...
        grds = np.zeros([int(np.ceil(float(ptchs.shape[0]) / float(parfact)) * parfact), np.prod(ptchs.shape[1:])],
                        dtype=np.complex64)
    ...
    for it in range(0, num_iter - 1, 2):
        ...
        print("it no: " + str(it) + " f_tot= " + str(ftot) + " f_lik= " + str(f_lik) + " f_dc (1e6)= " + str(
            f_dc / 1e6) + " |g_lik|= " + str(np.linalg.norm(g_lik)) + " |g_dc|= " + str(np.linalg.norm(g_dc)))
...
def TV_recon(imo, uspat):
    ...
    cfl.writecfl('/scratch_net/bmicdl02/Data/test/ksp',
                 np.fft.fftshift(np.fft.fft2((imo))) * uspat / np.sum(np.abs(imo)))
