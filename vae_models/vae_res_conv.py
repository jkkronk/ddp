...
def resblock_down_bilinear(inputs, filters_in, filters_out, scope_name, reuse, phase_train):
    h = tf.shape(inputs)[1]
    w = tf.shape(inputs)[2]
    with tf.variable_scope(scope_name, reuse=reuse):
        #tl.layers.set_name_reuse(reuse)
...
def resblock_up_bilinear(inputs, filters_in, filters_out, scope_name, reuse, phase_train):
    h = tf.shape(inputs)[1]
    w = tf.shape(inputs)[2]
    with tf.variable_scope(scope_name, reuse=reuse):
        #tl.layers.set_name_reuse(reuse)
