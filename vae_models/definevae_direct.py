...
def definevae(lat_dim=60, patchsize=28, batchsize=50, mode=[], vae_model=''):
     ...
     if useMixtureScale:
          ...
          if usebce:
               l2_loss_ = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=y_out, labels=x_inp__),
                                        reduction_indices=1)
     ...
     grd0 = tf.reduce_sum(tf.multiply((y_out - x_rec), y_out_prec), axis=1)
     grd20 = grd2[0]
