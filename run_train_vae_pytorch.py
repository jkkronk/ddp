...
from ConvVAE  import ConvVAE, train_vae, valid_vae
from dataloader_pytorch import fastMRI_patch
...
if __name__ == "__main__":
    ...
    # Load data
    print("Loading data...")
    train_dataset = fastMRI_patch(path, trainset_ratio=1, patchsize=img_size, modality='AXFLAIR_', train=True)
    train_data_loader  = data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=1)
    print('Train data loaded')

    validation_dataset = fastMRI_patch(path, trainset_ratio=1, patchsize=img_size, modality='AXFLAIR_', train=False)
    ...
    for epoch in range(epochs):
        ...
        if epoch % log_freq == 0: #and not epoch == 0:
            ...
            writer_valid.add_image('Batch of sampled images', normalize_tensor(torch.clamp(lat_batch_sample, 0, 1)), epoch, dataformats='NCHW')

            img_test = next(iter(valid_data_loader))
            img_test = img_test.to(device)
            img_re, __, __ = vae_model(img_test.double())
            writer_valid.add_image('Batch of original images', normalize_tensor(img_test), epoch, dataformats='NCHW')
            writer_valid.add_image('Batch of reconstructed images', normalize_tensor(torch.clamp(img_re, 0, 1)), epoch, dataformats='NCHW')
