...
def latent_layer_1(gf_dim):
    ...
    """
    return ResBlock_Down(gf_dim*4, gf_dim*8, act=False)
...
def decoder_layer(gf_dim):
    ...
    decoder = nn.Sequential(
        ...
        nn.Conv2d(gf_dim, gf_dim, kernel_size=3, stride=1, padding=0),
        nn.BatchNorm2d(gf_dim),
        nn.LeakyReLU(0.2),
        nn.Conv2d(gf_dim, 1, kernel_size=3, stride=1, padding=0)
        )
    return decoder
...
class ConvVAE(nn.Module):
    ...
    def sample(self, batch_size, device):
        ...
        sample = torch.randn(batch_size,self.gf_dim*8,2,2, dtype=torch.double).to(device) #batch_size, latent_size)
...
def train_vae(model, train_loader, device, optimizer, epoch):
    ...
    for batch_idx, data in enumerate(train_loader): #tqdm(enumerate(train_loader), total=len(train_loader), desc='train'):
        ...
        lat_loss, rec_loss = loss_function(recon_batch.double(), data.double(), mu, logvar)

        if epoch < 20:
            weight = 0.05 * epoch

        loss = torch.sum(weight * lat_loss + rec_loss) # Sum Latent loss and Reconstruction loss
...
def valid_vae(model, test_loader, device, epoch):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(test_loader): #tqdm(enumerate(test_loader), total=len(test_loader), desc='test'):
            data = data.to(device)
